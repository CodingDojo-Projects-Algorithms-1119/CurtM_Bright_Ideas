...
@app.route("/users/<idea_id>")
def user_profile(idea_id):
    mysql = connectToMySQL("bright_ideas")
    query = "SELECT ideas.idea_id, ideas.idea_content, users.name, users.alias, users.email FROM ideas JOIN users on ideas.author = users.user_id WHERE ideas.idea_id = %(i_id)s"
    data = {
        'i_id': idea_id
    }
    user = mysql.query_db(query, data)
